...
description: Virtauma's design system uses Tailwind CSS for simple theming and easy customization.
---
...
::note{to="/en/docs/getting-started/integrations/fonts"}
...
Virtauma's color system is designed around **semantic naming** rather than specific color values. This approach makes your UI more maintainable and allows for easy theme switching.

### Semantic colors

Virtauma provides semantic color aliases that describe the **purpose** of the color. Each alias is defined based on a color from your `@theme` configuration, which can be any color you define in addition to the [default Tailwind CSS palette](https://tailwindcss.com/docs/colors).
...
These semantic colors are available in the `color` prop of Virtauma components.
...
Finally, use this new color in components that support the `color` prop or [as a class](/en/docs/getting-started/theme/css-variables):
