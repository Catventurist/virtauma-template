...
description: Create multi-language documentation with Virtauma v4 built-in i18n support.
navigation:
  icon: i-lucide-globe
seo:
  description: Learn how to set up and manage multi-language documentation with
    Virtauma v4 built-in i18n support.
---

Virtauma v4 introduces **native internationalization support** based on the `@nuxtjs/i18n` module, allowing you to create documentation in multiple languages with automatic routing and content management.
...
To enable i18n in your Virtauma project, add the `@nuxtjs/i18n` module to your `nuxt.config.ts` and define your locales:
...
::warning
Virtauma overrides the `@nuxtjs/i18n` strategy to `prefix`.
::
...
Virtauma warns and skips any locale that does not exist in your `content/` directory. Missing locales are not registered.

::tip
This is especially helpful when you extend Virtauma and use `@nuxtjs/i18n` for the rest of your site, but only want the docs in a subset of languages.
::

Virtauma detects locales from your nuxt config:
...
If a user requests a missing docs locale, Virtauma redirects to the **default locale**.
